...
class EnemyCharacter(GameCharacter):
    ...
    def move_x(self, delta_time, width):
        ...
        elif self.y_f > width - self.rect.height:
            self.__collide_y(width - self.rect.height)

        self.rect.y = round(self.y_f)

    def __collide_x(self, collision_point_x, collide_y):
        """
        ...
        if self.direction_y == 0:
            ...
            if self.direction_x == 0:
                self.direction_x = random.choice([1.0, -1.0, 0])
            else:
                self.direction_x = random.choice([-self.direction_x, 0])
        self.move_to(collision_point_x * 2 - self.x_f)

        self.__collide_y(collide_y, collision_point_x)

    def __collide_y(self, collision_point_y, collide_x):
        """
        ...
        if self.direction_x == 0:
            self.direction_y = random.choice([1.0, -1.0])
        else:
            if self.direction_y == 0:
                self.direction_y = random.choice([-1.0, 1.0, 0])
            else:
                self.direction_y = random.choice([-self.direction_y, 0])
            ...
            if is_list_of_size(position, 2):
                ...
                if position[0] is not None:
                    if position[1] is not None:
                        self.__collide_x(position[0], position[1])

                # if position[1] is not None:
                #     if position[0] is not None:
                #         self.__collide_y(position[1], position[0])

...
class ScreenManager:
    ...
    def draw_health(self, group):
        for thing in group:
            if thing.tag == Tag.enemy:
                pygame.draw.rect(self.game_screen, (255, 0, 0), (thing.rect.x, thing.rect.y - 20, 50, 10))
                pygame.draw.rect(self.game_screen, (0, 255, 0), (thing.rect.x, thing.rect.y - 20, 50, 10))
    ...
    def loose_screen(self):
        """
        Screen to be shown when the player loses
        """

        # Update all the graphics
        self.paint_text_centered("You're Dead!")
